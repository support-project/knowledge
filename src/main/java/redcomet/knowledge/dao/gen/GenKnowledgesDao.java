...
public class GenKnowledgesDao extends AbstractDao {
	...
	public List<KnowledgesEntity> physicalSelectAll() { 
		String sql = SQLManager.getInstance().getSql("/redcomet/knowledge/dao/sql/KnowledgesDao/KnowledgesDao_physical_select_all.sql");
	...
	public KnowledgesEntity physicalSelectOnKey(Long knowledgeId) {
		String sql = SQLManager.getInstance().getSql("/redcomet/knowledge/dao/sql/KnowledgesDao/KnowledgesDao_physical_select_on_key.sql");
	...
	public List<KnowledgesEntity> selectAll() { 
		String sql = SQLManager.getInstance().getSql("/redcomet/knowledge/dao/sql/KnowledgesDao/KnowledgesDao_select_all.sql");
		return executeQuery(sql, KnowledgesEntity.class);
	}
	/**
	 * キーで1件取得 
	...
	public KnowledgesEntity selectOnKey(Long knowledgeId) {
		String sql = SQLManager.getInstance().getSql("/redcomet/knowledge/dao/sql/KnowledgesDao/KnowledgesDao_select_on_key.sql");
		return executeQueryOnKey(sql, KnowledgesEntity.class, knowledgeId);
	}
	/**
	 * 登録(データを生で操作) 
	...
	public KnowledgesEntity physicalInsert(KnowledgesEntity entity) {
		String sql = SQLManager.getInstance().getSql("/redcomet/knowledge/dao/sql/KnowledgesDao/KnowledgesDao_insert.sql");
	...
	public KnowledgesEntity physicalUpdate(KnowledgesEntity entity) {
		String sql = SQLManager.getInstance().getSql("/redcomet/knowledge/dao/sql/KnowledgesDao/KnowledgesDao_update.sql");
		executeUpdate(sql, 
			entity.getTitle()
			, entity.getContent()
			, entity.getPublicFlag()
	...
	public void physicalDelete(Long knowledgeId) {
		String sql = SQLManager.getInstance().getSql("/redcomet/knowledge/dao/sql/KnowledgesDao/KnowledgesDao_delete.sql");
