...
<div class="article_buttons">
    ...
    <div>
        <%
            if (request.getRemoteUser() != null) {
        %>
        <button type="button" class="btn btn-default btn_stock" data-toggle="modal" data-target="#stockModal">
        ...
        <a href="<%=request.getContextPath()%>/protect.knowledge/view/<%=jspUtil.out("knowledgeId")%>"
            class="btn btn-default btn_stock" role="button"> <i class="fa fa-star-o"></i>&nbsp; <%=jspUtil.label("knowledge.view.fav")%><%--(<%= jspUtil.label("knowledge.navbar.signin") %>) --%>
        </a>
        <%
            }
        %>
        <button class="btn btn-default btn_like" onclick="addlike(<%=jspUtil.out("knowledgeId")%>);">
    ...
    <div>
        <button class="btn btn-default btn_agenda" onclick="showAgenda();" id="buttonToc">
    ...
    <% if (jspUtil.is(TemplateLogic.TYPE_ID_EVENT, "typeId")) { %>
        <% if (jspUtil.logined()) { %>
            <button class="btn btn-default btn_col2 hide" id="btnParticipation">
                <i class="fa fa-user-plus"></i>&nbsp;
                <%=jspUtil.label("knowledge.view.label.participation")%>
            </button>
            <button class="btn btn-default btn_col2 hide" id="btnNonparticipation">
    ...
    <% } %>
        ...
        <a id="btnAnswerSurvey" class="btn btn-default btn_col2 hide" data-toggle="modal" href="#modalAnswerSurvey">
