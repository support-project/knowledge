...
public class KnowledgeLikedWebHookNotificationTest extends TestCommon {
    /** ログ */
    ...
    public static void setUpBeforeClass() throws Exception {
        ...
        config.setHook(WebhookConfigsEntity.HOOK_LIKED_KNOWLEDGE);
    ...
    @Test
    @Order(order=2)
    public void testSendLikedWebhook() throws Exception {
        int index = 1;
        LikeLogic.get().addLike(new Long(1), loginedUser, Locale.JAPAN);
        NotifyMailBat.main(null);
        
        List<WebhooksEntity> hooks = WebhooksDao.get().selectAll();
        Assert.assertEquals(index, hooks.size());
        Assert.assertEquals(WebhookConfigsEntity.HOOK_LIKED_KNOWLEDGE, hooks.get(hooks.size() - 1).getHook());
        ...
        String sendJSON = KnowledgeLikedWebHookNotification.get().createSendJson(hooks.get(hooks.size() - 1), KnowledgeUpdateWebHookNotificationTest.config);
        LOG.info(sendJSON);
        expected = new JsonParser().parse(new InputStreamReader(getClass().getResourceAsStream("webhook_send_liked.json"), "UTF-8"));
