...
public class NotifyLogic {
	...
	private boolean isToKnowledgeSave(LoginedUser loginuser, KnowledgesEntity knowledge) {
		...
		if (knowledge.getInsertUser().intValue() == loginuser.getUserId().intValue()) {
			//TODO 自分が登録した場合は、通知する必要無し？いったんは通知する
			// return false;
		}
	...
	/**
	 * 指定のナレッジは、自分宛てのナレッジかどうかを判定する
	 * 自分宛ては以下の場合を言う
	 * ・公開区分が「公開」でかつ、「公開」でも通知する設定になっている場合
	 * ・公開区分が「保護」でかつ、宛先に自分が入っている場合
	 * 
	 * @param loginuser
	 * @param knowledge
	 * @param entity
	...
	/**
	 * 指定のコメントの更新で通知するかどうかを判定し、通知する場合は通知するメッセージを取得する
	 * @param loginuser
	 * @param locale
	 * @param comment
	 * @return
	 */
	public MessageResult getSaveCommentMessage(LoginedUser loginuser, Locale locale, CommentsEntity comment) {
		NotifyConfigsDao dao = NotifyConfigsDao.get();
		NotifyConfigsEntity entity = dao.selectOnKey(loginuser.getUserId());
		if (!flagCheck(entity.getNotifyDesktop())) {
			//デスクトップ通知対象外
		...
		if (flagCheck(entity.getMyItemComment()) && knowledge.getInsertUser().intValue() == loginuser.getUserId().intValue()) {
