			...
			<div class="article_buttons">
				...
				<% } else { %>
					<a href="<%= request.getContextPath() %>/protect.knowledge/view/<%= jspUtil.out("knowledgeId") %>"
						class="btn btn-info btn_stock" role="button">
						<i class="fa fa-star-o"></i>&nbsp;
...
<%-- コメント表示 --%>
	<% List commentList = jspUtil.getValue("comments", List.class);
		...
		<hr/>
		<h5 id="comments"><i class="fa fa-comments-o"></i>&nbsp;<%= jspUtil.label("knowledge.view.comment.label") %></h5>
...
<%-- コメント登録 --%>
<hr/>
<h4 class="sub_title"><i class="fa fa-comment-o"></i>&nbsp;<%= jspUtil.label("knowledge.comment.add") %></h4>
	<% if (request.getRemoteUser() != null) { %>
